...
sample_number <- length(group_inf_df$V1)

gc_files <- file.path(gc_dir, paste(group_inf_df$V1, "gc.txt", sep = '.'))
...
for (i in seq(sample_number)) {
  ...
  each_sample_gc_df$sample <- group_inf_df$V1[i]
  each_sample_gc_df[is.na(each_sample_gc_df)] <- 0
  gc_file_list[[i]] <- each_sample_gc_df
  each_sample_out_name <- paste(group_inf_df$V1[i], 'gc_distribution.line', sep = '.')
