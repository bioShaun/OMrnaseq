...
## log transform
PCA_data_mat <- log10(PCA_data_mat + 1)
PCA <- prcomp(PCA_data_mat) #no scale
Summary_PCA <- summary(PCA)
sample_num = dim(PCA_data)[2] - 1
PCA_data <- as.data.frame(PCA$x[,1:sample_num])
sample_name <- rownames(PCA$x)
calc_color <- colorRampPalette(calc_pal()(12))(length(sample_name))
PCA_data$Sample <- sample_name 
PCA_plot <- function(PCA_data){
  p <- ggplot(PCA_data,aes(PC1,PC2))+
    geom_point(aes(colour = Sample),size = rel(3.0))+
    ...
    theme_onmath()+
    scale_color_manual(values = calc_color) +
    labs(title = "PCA",
